...
class SaleEditor extends Controller
{
	...
	public function index()
	{
		...
		*/
		...
		return BulkCurrencyEditorCURCY()->view('dashboard.sales')
		->with($data)
	...
	public function store()
	{
		...
		if($selected_products != null && !empty($selected_products)){
			...
			foreach ($selected_products as $post_id) {
				...
				$existingPrices = get_post_meta($post_id, '_sale_price_wmcp', true);
				...
				$resp += update_post_meta($post_id, '_sale_price_wmcp', $updatedPricesJSON);
			}
		}
		//back to main page
		$this->redirect('admin.php?page=bulkcurrencyeditorcurcy_1');
	}
